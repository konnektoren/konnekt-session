...
Significant architectural and technology decisions are documented as ADRs following the **Nygard ADR format**:

[cols="1,3,2", options="header"]
|===
|ID |Title |Status

|xref:adr/0001-use-rust-for-implementation.adoc[ADR-0001]
|Use Rust for Implementation
|Accepted

|xref:adr/0002-use-yew-for-frontend.adoc[ADR-0002]
|Use Yew for Frontend Framework
|Accepted

|xref:adr/0003-use-matchbox-for-signaling.adoc[ADR-0003]
|Use Matchbox for WebRTC Signaling
|Accepted
|===

See the full **xref:adr/README.adoc[ADR Index]** for details on context, alternatives considered, and consequences of each decision.
...
|xref:04-strategize.adoc[Strategize]
...
|Type safety, WASM-first, memory safety without GC, excellent domain modeling.
|**xref:adr/0001-use-rust-for-implementation.adoc[ADR-0001]**

|**Yew for frontend**
|React-like components, hooks, zero-cost integration with domain logic, type-safe props.
|**xref:adr/0002-use-yew-for-frontend.adoc[ADR-0002]**

|**Matchbox for signalling**
|Rust-native WebRTC, minimal infrastructure, game-specific API, reliable/unreliable channels.
|**xref:adr/0003-use-matchbox-for-signaling.adoc[ADR-0003]**

|**Client-side Ed25519 keys**
|Deterministic identity from name+password, enables reconnection, prevents impersonation.
|**xref:adr/0004-use-client-side-key-generation-for-identity.adoc[ADR-0004]**
...
TIP: Technology decisions (Rust, Yew, Matchbox) are documented with full context, alternatives, and trade-offs in the **xref:adr/README.adoc[ADR section]**.
...
* **ADR** – https://adr.github.io/ (Nygard format)

=== Internal Documentation

* **xref:adr/README.adoc[Architecture Decision Records]** – Technology choices and trade-offs
* **xref:01-understand.adoc[DDD Process: Understand]** – Business model and constraints
* **xref:04-strategize.adoc[DDD Process: Strategize]** – Core domain investment strategy
